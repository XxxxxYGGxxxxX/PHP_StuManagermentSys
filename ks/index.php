...
require './page_function.php'; // 引入分页函数库

// 函数：生成排序SQL
function make_sort_sql($order, $sort) {
    //允许排序的字段
    $fields = array('id', 'name', 'class', 'sex','grade','date_of_birth', 'date_of_enter');
    // 判断$order是否存在于合法字段列表中
    if (in_array($order, $fields)) {
        // 判断$sort是否为合法值
        if (in_array($sort, array('asc', 'desc'))) {
            return "order by `p_LP_$order` $sort";
        }
    }
    return '';
}
...
function make_search_sql($keyword) {
    ...
    return "and p_LP_name like '%$keyword%'";
}
...
// 分页功能
$page_size = 10; // 每页显示5条信息
$page_count = db_fetch_column('select count(*) from php_LP'); // 查询所有记录条数
...
// 准备SQL语句
$sql = "select * from php_LP left join php_class on p_LP_class=p_class_id where 1=1 $where_sql $order_sql $limit_sql";
